...
from application.port.inbound.socketio_inbound_port import SocketIOInboundPort
from application.port.outbound.socketio_outbound_port import SocketIOOutboundPort
...
class BroadcastStreamUseCase(SocketIOInboundPort):
    ...
    async def handle_response_client_metadata(self, sid, dto:ResponseClientMetadataDTO) -> None:
        if dto.client_type == "stream_service" :
