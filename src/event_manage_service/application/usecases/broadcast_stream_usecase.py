...
class BroadcastStreamUseCase(EventSubscriber):
    ...
    async def handle_client_connect(self, sid) -> None:
        await self.event_publisher.request_client_metadata(sid)
    ...
    async def handle_response_client_metadata(self, sid, dto:ResponseClientMetadataDTO) -> None:
        if dto.client_type == "stream-service" :
            ...
            await self.event_publisher.set_stream_service(sid)
        elif dto.client_type == "user" :
            await self.event_publisher.add_client_to_room(sid, self.rooms.USER_ROOM)

    async def handle_client_disconnect(self, sid: str) -> None:
        await self.event_publisher.remove_client_from_all_rooms(sid)

    async def handle_capture_start(self, sid: str) -> None:
        await self.event_publisher.send_capture_start_command()
        
        client_ip = self.event_publisher.get_sid_ip(sid)
    ...
    async def handle_capture_stop(self, sid: str) -> None:
        await self.event_publisher.send_capture_stop_command()
    ...
    async def handle_join_streaming_room(self, sid: str) -> None:
        await self.event_publisher.add_client_to_room(sid, self.rooms.STREAMING_ROOM)
    ...
    async def handle_leave_streaming_room(self, sid: str) -> None:
        await self.event_publisher.remove_client_from_room(sid, self.rooms.STREAMING_ROOM)
    ...
    async def handle_video_frame_relay(self, dto: VideoFrameFromServiceDTO) -> None:
        await self.event_publisher.broadcast_video_frame(dto)
