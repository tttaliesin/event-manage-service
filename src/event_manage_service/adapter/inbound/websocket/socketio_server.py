...
class SocketIOServer:
    ...
    def resister_event(self):
        ...
        async def connect(sid, environ):
            ...
            await self.event_subscriber.handle_client_connect(sid)
        ...
        async def response_client_metadata(sid, data: dict[str, any]):
            ...
            try:
                ...
                await self.event_subscriber.handle_response_client_metadata(sid, dto)
            except ValueError:
        ...
        async def disconnect(sid, reason):
            ...
            await self.event_subscriber.handle_client_disconnect(sid)
        ...
        async def start_capture(sid):
            ...
            await self.event_subscriber.handle_capture_start(sid)
        ...
        async def video_frame_relay(sid, data):
            ...
            try:
                ...
                await self.event_subscriber.handle_video_frame_relay(sid, dto)    
            except ValueError:
                logger.error(f"{sid} client의 응답{data}의 DTO 변환 중 value error가 발생했습니다.")
            except :
                logger.error(f"{sid} client의 응답{data}의 DTO 변환에 실패했습니다.")
        ...
        async def request_capture_status(sid):
            ...
            await self.event_subscriber.handle_request_capture_status(sid)
        ...
        async def broadcast_capture_status(sid, data: dict[str, any]):
            ...
            try:
                ...
                await self.event_subscriber.handle_broadcast_capture_status(sid, dto)
            except ValueError:
                logger.error(f"{sid} client의 응답{data}의 DTO 변환 중 value error가 발생했습니다.")
            except :
                logger.error(f"{sid} client의 응답{data}의 DTO 변환에 실패했습니다.")